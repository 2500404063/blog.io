...
[ol
Can't be copied, only can be moved.
No referrence count.
Only can be initialized by `unique_ptr<int> p(new int)` .
ol]
...
[ol
Can copy a shared_ptr object.
Copying a shared_ptr object **WON'T** increase referrence count.
Cannot change value that pointer points to, but can read.
ol]
